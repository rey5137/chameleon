 ...
 */
class ViewStyler implements Styler<View> {

    private static final SimpleArrayMap<String, Typeface> sCachedFonts = new SimpleArrayMap<>();
    private static final String PREFIX_ASSET = "asset:";

    @SuppressWarnings("ResourceType")
    @Override
    public void applyStyle(View v, int styleRes) {
        TypedArray a = v.getContext().obtainStyledAttributes(null, R.styleable.View, 0, styleRes);
        int leftPadding = -1;
        int topPadding = -1;
        int rightPadding = -1;
        int bottomPadding = -1;
        ...
        if(v instanceof TextView)
            applyStyle((TextView)v, styleRes);
    }
    ...
    /**
     * @param familyName if start with 'asset:' prefix, then loadTypeface font from asset folder.
     * @return
     */
    private Typeface loadTypeface(Context context, String familyName, int style) {
    ...
    private void applyStyle(TextView v, int styleRes){
        ...
        TypedArray a = v.getContext().obtainStyledAttributes(null, R.styleable.TextViewAppearance, 0, styleRes);
        ...
        a = v.getContext().obtainStyledAttributes(null, R.styleable.TextView, 0, styleRes);
        ...
        Typeface tf = null;
        if (fontFamily != null) {
            tf = loadTypeface(v.getContext(), fontFamily, styleIndex);
            if (tf != null)
                v.setTypeface(tf);
        }
        if(tf != null) {
        ...
        if(v instanceof AutoCompleteTextView)
            applyStyle((AutoCompleteTextView)v, styleRes);
    }
