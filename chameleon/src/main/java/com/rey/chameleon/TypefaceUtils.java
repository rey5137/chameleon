...
public final class TypefaceUtils {
    ...
    private static final Map<Typeface, ChameleonTypefaceSpan> sCachedSpans = new HashMap<Typeface, ChameleonTypefaceSpan>();
    ...
    public static Typeface load(final AssetManager assetManager, final String filePath) {
        synchronized (sCachedFonts) {
            try {
                ...
                Log.w("Chameleon", "Can't create asset from " + filePath + ". Make sure you have passed in the correct path and file name.", e);
     ...
     */
    public static ChameleonTypefaceSpan getSpan(final Typeface typeface) {
        if (typeface == null) return null;
        synchronized (sCachedSpans) {
            if (!sCachedSpans.containsKey(typeface)) {
                final ChameleonTypefaceSpan span = new ChameleonTypefaceSpan(typeface);
