...
import com.rey.chameleon.theme.ViewStyleDelegate;
...
class ChameleonFactory {
    ...
    protected static int[] getStyleForTextView(TextView view) {
        ...
        if (styleIds[0] == -1) {
            ...
            styleIds[0] = ChameleonConfig.get().getClassStyles().containsKey(view.getClass())
                    ? ChameleonConfig.get().getClassStyles().get(view.getClass())
    ...
    protected static boolean parentIsToolbarV7(View view) {
        return ChameleonUtils.canCheckForV7Toolbar() && view.getParent() != null && (view.getParent() instanceof Toolbar);
    }
    ...
    public ChameleonFactory(int attributeId) {
    ...
    public View onViewCreated(View view, Context context, AttributeSet attrs) {
        if (view != null && view.getTag(R.id.chameleon_tag_id) != Boolean.TRUE) {
            onViewCreatedInternal(view, context, attrs);
            int styleId = ThemeManager.getStyleId(context, attrs, 0, 0);
            if(styleId != 0)
                view.addOnAttachStateChangeListener(new ViewStyleDelegate(view, styleId));
            view.setTag(R.id.chameleon_tag_id, Boolean.TRUE);
        }
        return view;
    }
    ...
    void onViewCreatedInternal(View view, final Context context, AttributeSet attrs) {
        if (view instanceof TextView) {
            ...
            if (TextUtils.isEmpty(textViewFont)) {
                ...
                if (styleForTextView[1] != -1)
                    textViewFont = ChameleonUtils.pullFontPathFromTheme(context, styleForTextView[0], styleForTextView[1], mAttributeId);
                else
                    textViewFont = ChameleonUtils.pullFontPathFromTheme(context, styleForTextView[0], mAttributeId);
            }
            ...
            ChameleonUtils.applyFontToTextView(context, (TextView) view, ChameleonConfig.get(), textViewFont, deferred);
        }

        // AppCompat API21+ The ActionBar doesn't inflate default Title/SubTitle, we need to scan the
        // Toolbar(Which underlies the ActionBar) for its children.
        if (ChameleonUtils.canCheckForV7Toolbar() && view instanceof Toolbar) {
        ...
        } else if (ChameleonConfig.get().isCustomViewTypefaceSupport() && ChameleonConfig.get().isCustomViewHasTypeface(view)) {
    ...
    private Typeface getDefaultTypeface(Context context, String fontPath) {
        if (TextUtils.isEmpty(fontPath)) {
            fontPath = ChameleonConfig.get().getFontPath();
        }
        if (!TextUtils.isEmpty(fontPath)) {
    ...
    private String resolveFontPath(Context context, AttributeSet attrs) {
        ...
        String textViewFont = ChameleonUtils.pullFontPathFromView(context, attrs, mAttributeId);

        // Try view style attributes
        if (TextUtils.isEmpty(textViewFont)) {
            textViewFont = ChameleonUtils.pullFontPathFromStyle(context, attrs, mAttributeId);
        }

        // Try View TextAppearance
        if (TextUtils.isEmpty(textViewFont)) {
            textViewFont = ChameleonUtils.pullFontPathFromTextAppearance(context, attrs, mAttributeId);
        }
    ...
    private static class ToolbarLayoutListener implements ViewTreeObserver.OnGlobalLayoutListener {
        ...
        private final WeakReference<ChameleonFactory> mCalligraphyFactory;
        private final WeakReference<Context> mContextRef;
        private final WeakReference<Toolbar> mToolbarReference;
        private final CharSequence originalSubTitle;

        private ToolbarLayoutListener(final ChameleonFactory chameleonFactory,
                                      final Context context, Toolbar toolbar) {
            mCalligraphyFactory = new WeakReference<>(chameleonFactory);
        ...
        @Override public void onGlobalLayout() {
            ...
            final ChameleonFactory factory = mCalligraphyFactory.get();
